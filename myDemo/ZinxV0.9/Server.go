...
func (h *HelloRouter) Handle(request ziface.IRequest) {
	...
	fmt.Printf("recv from client, msgId: %d, data: %s\n", request.GetMsgID(), request.GetData())
	err := request.GetConnection().SendMsg(request.GetMsgID(), []byte("Hello Zinx V0.9"))
	if err != nil {
		fmt.Println(err)
	}
}
...
func (q *QuitRouter) Handle(request ziface.IRequest) {
	...
	fmt.Printf("recv from client, msgId: %d, data: %s\n", request.GetMsgID(), request.GetData())
	err := request.GetConnection().SendMsg(request.GetMsgID(), []byte("Quit Zinx V0.9"))
	if err != nil {
		fmt.Println(err)
	}
}
...
func ConnStart(conn ziface.IConnection) {
	...
	err := conn.SendMsg(202, []byte("Hello Zinx V0.9"))
	if err != nil {
		fmt.Println(err)
	}
}
